...
Each app keeps its context in AGENTS.md, with CLAUDE.md as a one-line `@AGENTS.md`
import. The four Next apps invert it: their AGENTS.md is generated by Next and carries
framework directives, so their context sits in CLAUDE.md below that import — keep the
import line first when editing. When bumping framework versions, update the Stack
column below — it drifts stale otherwise.
