...
export const fern = (
  ...
  for (let f = 0; f < fronds; f++) {
    ...
    const len =
      h * (0.72 + (1 - Math.abs(spread)) * 0.28) * (0.9 + rand() * 0.15);
    ...
    for (let i = 1; i <= pairs; i++) {
      ...
      const [px, py] = at(t);
      const tangent =
        (Math.atan2(-len, 2 * bend * len * t) * 180) / Math.PI + 90;
...
export const bush = (
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const oy = -rand() * w * 0.2;
    const color =
      [p.flora.leaf, p.flora.leafLit, p.flora.leaf, p.flora.grass][i % 4] ??
      p.flora.leaf;
