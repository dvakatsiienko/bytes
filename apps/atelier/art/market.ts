...
/** a lit window glows, an unlit one is dark by night and sky-blue by day */
const windowGlass = (p: Palette, isLit: boolean) => {
  if (isLit) return '#FFC766';
  return p.isNight ? '#2A3052' : p.cabin.window;
};
...
const awning = (
  ...
  for (let i = 0; i < stripes; i += 2)
    d += `<rect x="${n(x + i * sw)}" y="${y}" width="${n(sw)}" height="30" fill="${accent}"/>`;
  let scallop = `M${x} ${y + 30}`;
  for (let i = 0; i < stripes; i++)
    scallop += `a${n(sw / 2)} ${n(sw / 2.4)} 0 0 0 ${n(sw)} 0`;
...
const goods = (
  ...
  for (const shelfY of [y + 52, y + 104]) {
    ...
    while (cx < x + 96) {
      ...
      if (kind === 0)
        out += `<rect x="${n(cx)}" y="${n(shelfY - h)}" width="${n(w)}" height="${n(h)}" rx="2" fill="${c}"/>`;
      if (kind === 1)
        out += `<circle cx="${n(cx + w / 2)}" cy="${n(shelfY - w / 2)}" r="${n(w / 2)}" fill="${c}"/>`;
      if (kind === 2)
        out += `<path d="M${n(cx)} ${shelfY}L${n(cx + w / 2)} ${n(shelfY - h)}L${n(cx + w)} ${shelfY}Z" fill="${c}"/>`;
      cx += w + 4;
    }
  }
  return out;
...
const shop = (p: Palette, app: AppName, i: number, x: number, w: number) => {
  ...
  const upper = [0, 1]
    .map((k) => {
      ...
      return `<rect x="${wx - 4}" y="${top + 44}" width="44" height="58" rx="4" fill="${p.room.trim}"/><rect x="${wx}" y="${top + 48}" width="36" height="50" rx="2" fill="${windowGlass(p, lit)}"/><path d="M${wx + 18} ${top + 48}v50" stroke="${p.room.trim}" stroke-width="3"/><rect x="${wx - 6}" y="${top + 100}" width="48" height="8" rx="2" fill="${p.room.floor}"/>${rosette(wx + 8, top + 100, 6, accent, p.flora.cream, p, k * 20)}${rosette(wx + 28, top + 99, 7, p.flora.coral, p.flora.marigold, p)}`;
    })
    .join('');
  const winX = x + 18;
  const winY = BASE - 168;
  ...
  return {
    facade: `<g><g filter="url(#liftHi)">${roof}${body}</g><g filter="url(#lift)">${upper}${display}${door}</g>${awning(p, winX - 6, winY - 40, 134, accent)}${pots}</g>`,
    sign,
  };
};
...
const bunting = (p: Palette) => {
  ...
  spans.forEach(([x0, y0, x1, y1], s) => {
    ...
    for (let k = 1; k < 9; k++) {
      const t = k / 9;
      const bx =
        (1 - t) ** 2 * x0 + 2 * (1 - t) * t * ((x0 + x1) / 2) + t ** 2 * x1;
      const by =
        (1 - t) ** 2 * y0 +
        2 * (1 - t) * t * ((y0 + y1) / 2 + sag * 2) +
        t ** 2 * y1;
...
const street = (p: Palette) => {
  ...
  for (let row = 0; row < 6; row++) {
    ...
    const sw = 34 + row * 8;
    for (let x = -20 + (row % 2) * (sw / 2); x < W; x += sw + 6)
      stones += `<rect x="${n(x + rand() * 4)}" y="${n(y)}" width="${sw}" height="${12 + row * 1.6}" rx="${6 + row}" fill="${p.stone.lit}" opacity=".75"/>`;
  }
  const shade = `<rect y="${BASE + 18}" width="1250" height="22" fill="${p.shadow.color}" opacity=".1"/>`;
  const moss = Array.from({ length: 14 }, (_, i) =>
    grassTuft(60 + i * 110 + rand() * 40, BASE + 20, 10, p.flora.grass, i + 60),
  ).join('');
...
export const marketLayers = (
  ...
  const fireflies =
    p.isNight && hasFireflies
      ? Array.from({ length: 26 }, () =>
          glowDot(40 + rand() * 1500, 120 + rand() * 360, 1.1 + rand() * 1.3),
        ).join('')
      : '';
  ...
  return [
    { body: `${sky}${stars}`, depth: 9, name: 'sky' },
    {
      body: `<g opacity="${p.isNight ? 0.3 : 1}">${cloud(300, 70, 130, p, 1)}${cloud(1180, 60, 110, p, 2)}${cloud(1480, 150, 90, p, 3)}</g>`,
      depth: 8.5,
      name: 'clouds',
    },
    { body: castle, depth: 6, name: 'castle' },
    { body: street(p), depth: 3, name: 'street' },
    { body: shops.map((s) => s.facade).join(''), depth: 2.6, name: 'shops' },
    { body: shops.map((s) => s.sign).join(''), depth: 2.2, name: 'signs' },
    { body: bunting(p), depth: 2, name: 'bunting' },
    {
      body: [plaque, lamppost(p, 1272), cart(p, 1470)].join(''),
      depth: 1.4,
      name: 'props',
    },
    {
    ...
    { body: fireflies, depth: 0, name: 'foreground' },
  ];
};

export const market = (p: Palette) =>
  `${defs(p)}<clipPath id="mframe"><rect width="${W}" height="${H}" rx="22"/></clipPath><g clip-path="url(#mframe)">${marketLayers(
    p,
  )
    .map((l) => l.body)
