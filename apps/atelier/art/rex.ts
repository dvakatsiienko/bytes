...
const shape = {
  ...
  thigh:
    'M98 92C106 78 134 78 140 96C144 110 136 122 126 128L130 146L146 146L148 151L110 151L113 128C101 120 94 104 98 92Z',
};
...
const hanging = (p: Palette, kind: 'lantern' | 'basket') => {
  const string =
    '<path d="M3 80Q2 90 3 96" stroke="#00000055" stroke-width="1"/>';
  if (kind === 'lantern') {
...
export const rex = ({
  ...
  const stripes = at(
    ...
    (sx, sy) =>
      `<path d="M${sx} ${sy}q4 8 0 16" stroke="${dark}" stroke-width="3" fill="none" stroke-linecap="round" opacity=".7"/>`,
  );
  const freckles = at(
    [
      [30, 80],
    ...
    (fx, fy) =>
      `<circle cx="${fx}" cy="${fy}" r="1.5" fill="${dark}" opacity=".6"/>`,
  );
  ...
  const headLight = `<path fill="${lit}" d="M160 50C160 36 166 24 176 19L212 13C218 12 222 14 224 17L178 23C170 28 166 38 164 50Z" opacity=".8"/>${at(
    ...
    (hx, hy) =>
      `<circle cx="${hx}" cy="${hy}" r="1.4" fill="${dark}" opacity=".6"/>`,
  ...
  const rim = p.isNight
    ? `<path d="M8 78C40 70 78 58 108 51C128 46 146 45 158 50" stroke="#F2994A" stroke-width="1.6" fill="none" opacity=".35"/>`
    : '';
