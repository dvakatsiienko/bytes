...
const nightSpecks = (
  ...
  return Array.from(
    { length: 14 },
    () =>
      `<circle cx="${n(x + rand() * w)}" cy="${n(y + rand() * h)}" r="${n(0.6 + rand())}" fill="#FFF3C4" opacity=".8"/>`,
  ).join('');
};
...
export const tourFrame = (p: Palette) => {
  ...
  const glow = p.isNight
    ? `<circle cx="230" cy="190" r="150" fill="url(#warmth)" opacity=".8"/>`
    : '';
  const body = [
    wall,
    glow,
    ...
    shelf,
    tools,
    plaque(p, 386, 30, 'frame', 'the machine as data'),
    bench,
    bits,
    lamp(p, 170, 236, 1.08),
    dym(p, 170 + 104 * 1.08, 236 - 60 * 1.08, p.isNight, 0.95),
  ].join('');
...
export const tourBytes = (p: Palette) => {
  const sky =
    skyRect(p, 0, 0, W, H, 'tbSky') + nightSpecks(p, 180, 10, 300, 110, 9);
  const walls = p.isNight
    ? ['#454B70', '#3F4A62', '#4C4566', '#443F66']
    : ['#E4ECF3', '#DFEADF', '#F0E4E2', '#E5E2F0'];
  const roofs = [
  ...
  const wall = `<g filter="url(#liftHi)"><rect x="-4" y="0" width="150" height="${H}" fill="${p.stoneWall.face}"/>${Array.from(
    { length: 11 },
    (_, r) => {
      const y = 12 + r * 26;
      return `<path d="M0 ${y}H146" stroke="${p.stoneWall.joint}" stroke-width="2"/>${Array.from({ length: 3 }, (_stone, c) => `<path d="M${(r % 2 ? 26 : 0) + c * 52} ${y}v26" stroke="${p.stoneWall.joint}" stroke-width="2"/>`).join('')}`;
    },
  ).join(
    '',
  )}<rect x="138" y="0" width="10" height="${H}" fill="${p.stoneWall.lit}"/><rect x="30" y="150" width="70" height="80" rx="4" fill="${p.room.trim}"/><rect x="36" y="156" width="58" height="68" fill="${p.isNight ? '#FFC766' : p.cabin.window}"/><path d="M65 156v68M36 190h58" stroke="${p.room.trim}" stroke-width="4"/><rect x="24" y="228" width="82" height="12" rx="3" fill="${p.room.floor}"/></g>${rosette(40, 226, 9, p.flora.coral, p.flora.marigold, p)}${rosette(64, 222, 11, p.flora.lapis, p.flora.cream, p, 12)}${rosette(90, 226, 9, p.flora.marigold, p.flora.coral, p, 24)}`;
  ...
  const glow = p.isNight
    ? `<circle cx="120" cy="190" r="90" fill="url(#warmth)" opacity=".7"/>`
    : '';
  const body = [
    sky,
    tower,
    ...
    cobbles,
    wall,
    bracket(p, 146, 58, 238),
    sign,
    lantern(p, 380, 58, 1.1),
  ].join('');
  return frameClip(body, p);
};

export const tileSize = { h: H, w: W };
