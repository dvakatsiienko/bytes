...
const town = (p: Palette, hasGlow = true) => {
  ...
  for (const [x, w, h, roof] of houses) {
    const base =
      300 -
      (Math.abs(x - 1140) < 140 ? (140 - Math.abs(x - 1140)) * 0.2 : 0) +
      8;
  ...
  const glow =
    p.isNight && hasGlow
      ? `<ellipse cx="1140" cy="270" rx="200" ry="70" fill="url(#warmth)" opacity=".7"/>`
      : '';
...
const cabin = (p: Palette, x: number, base: number, hasSmoke = true) => {
  ...
  for (let i = 0; i < rows; i++) {
    ...
    for (const ex of [x - 4, x + w + 4])
      logs += `<circle cx="${ex}" cy="${y + logH / 2}" r="7.4" fill="${p.cabin.logEnd}"/><circle cx="${ex}" cy="${y + logH / 2}" r="3.6" fill="none" stroke="${p.cabin.ring}" stroke-width="1.2"/>`;
  }
  const peak = top - 70;
  const gable = `<path d="M${x + 4} ${top}L${x + w / 2} ${peak + 8}L${x + w - 4} ${top}Z" fill="${p.cabin.logLit}"/><path d="${Array.from(
    { length: 5 },
    (_, i) => {
      const y = top - 12 * (i + 1);
  ...
  const pile = Array.from({ length: 6 }, (_, i) => {
    // three logs, then two on them, then one on top
    const [row, col] = pileSlots[i] ?? [0, 0];
...
const bridge = (p: Palette, x: number, y: number) => {
  let planks = '';
  for (let i = 0; i < 9; i++)
    planks += `<rect x="${x - 40 + i * 9}" y="${n(y - 6 - Math.sin((i / 8) * Math.PI) * 8)}" width="8" height="12" rx="1.5" fill="${p.bridge.plank}"/>`;
  const rail = (dy: number) =>
    `<path d="M${x - 44} ${y - 4 + dy}Q${x} ${y - 30 + dy} ${x + 44} ${y - 4 + dy}" stroke="${p.bridge.rail}" stroke-width="3.2" fill="none" stroke-linecap="round"/>`;
  const posts = [-40, -14, 14, 40]
    .map(
      (dx) =>
        `<path d="M${x + dx} ${n(y - 4 - Math.cos((dx / 44) * (Math.PI / 2)) * 8)}v-18" stroke="${p.bridge.rail}" stroke-width="3" stroke-linecap="round"/>`,
    )
    .join('');
...
const foreground = (p: Palette, side: 'left' | 'right') => {
  ...
  return [
    ...
    flip
      ? berries(x(134), 548, 9, p, 5)
      : bud(x(136), 556, 20, p.flora.lilac, p, -8),
  ].join('');
};
...
export const valley = (p: Palette) => {
  ...
  const body = [
    ...
    `<path d="${ridge({ amp: 26, floor: 420, seed: 8, waves: [520, 230], x0: -20, x1: W, y: 262 })}" fill="${p.mountain[0]}"/>`,
    `<path d="${ridge({ amp: 18, floor: 420, seed: 9, waves: [380, 170], x0: -20, x1: W, y: 292 })}" fill="${p.mountain[1]}"/>`,
    town(p),
    `<g filter="url(#lift)"><path d="${ridge({ amp: 10, floor: 460, seed: 10, waves: [300, 120], x0: -20, x1: W, y: 322 })}" fill="${p.ridge.far}"/>${pineRow(-10, 960, (x) => 322 + Math.sin(x / 90) * 6, 30, p.ridge.pines, 12)}${pineRow(1330, W + 10, (x) => 324 + Math.sin(x / 90) * 6, 30, p.ridge.pines, 13)}</g>`,
    `<path filter="url(#lift)" d="${ridge({ amp: 9, floor: H, seed: 14, waves: [420, 190], x0: -20, x1: W, y: 356 })}" fill="${p.hill.back}"/>`,
    `<path filter="url(#lift)" d="M-20 ${backHillY(0) + 40}${smooth(Array.from({ length: 18 }, (_, i) => [i * 100 - 20, 404 + Math.sin(i * 0.9) * 8] as const)).replace(LEADING_MOVE, '')}L1680 ${H}L-20 ${H}Z" fill="${p.hill.mid}"/>`,
    `<path filter="url(#lift)" d="${ridge({ amp: 12, floor: H, seed: 15, waves: [500, 210], x0: -20, x1: W, y: 470 })}" fill="${p.hill.front}"/>`,
  ...
  const rows = [0, 1, 2]
    .map((r) => {
      ...
      const heads = Array.from(
        { length: 5 },
        (_, i) =>
          `<circle cx="${x + 18 + i * 30 + (r % 2) * 14}" cy="${ry - 4}" r="${7 + (i % 2)}" fill="${i % 2 ? p.flora.leafLit : p.flora.leaf}"/>`,
      ).join('');
      return `<rect x="${x}" y="${ry - 4}" width="170" height="10" rx="5" fill="${p.room.floorShade}" opacity=".55"/>${heads}`;
    })
    .join('');
  const sunflower = `<path d="M${x + 186} ${y + 30}V${y - 70}" stroke="${p.flora.stem}" stroke-width="3"/><path transform="translate(${x + 186} ${y - 20}) rotate(-50)" d="${comma(20, 9)}" fill="${p.flora.leaf}"/>${rosette(x + 186, y - 76, 13, p.flora.marigold, p.room.floorShade, p)}`;
  return `<g filter="url(#lift)">${rows}</g>${sunflower}`;
};

const clothesline = (p: Palette, x0: number, x1: number, y: number) => {
  const mid = (x0 + x1) / 2;
  const scarf = p.isNight
    ? ''
    : `<g filter="url(#lift)"><path d="M${mid - 40} ${y + 12}h30v62l-6 8l-6 -8l-6 8l-6 -8l-6 8Z" fill="${p.dino.scarf}"/><path d="M${mid - 40} ${y + 30}h30M${mid - 40} ${y + 52}h30" stroke="${p.dino.scarfStripe}" stroke-width="3"/></g>`;
...
export const homesteadLayers = (
  p: Palette,
  { hasFireflies = true, hasSmoke = true, hasGlow = true } = {},
): readonly Layer[] => {
  const rand = seeded(7);
  ...
  return [
    {
      body: [
        sky(p),
        stars(p),
        p.isNight ? moon(p, 1360, 96, 32) : sun(p, 1360, 100, 38),
      ].join(''),
      depth: 9,
      name: 'sky',
    },
    {
      body: `<g opacity="${p.isNight ? 0.35 : 1}">${cloud(260, 96, 140, p, 1)}${cloud(700, 64, 110, p, 2)}${cloud(1020, 140, 120, p, 3)}</g>`,
      depth: 8.5,
      name: 'clouds',
    },
    {
      body: `<path d="${ridge({ amp: 26, floor: 420, seed: 8, waves: [520, 230], x0: -20, x1: W, y: 262 })}" fill="${p.mountain[0]}"/><path d="${ridge({ amp: 18, floor: 420, seed: 9, waves: [380, 170], x0: -20, x1: W, y: 292 })}" fill="${p.mountain[1]}"/>`,
      depth: 7,
      name: 'mountains',
    },
    { body: town(p, hasGlow), depth: 6, name: 'town' },
    {
      body: `<g filter="url(#lift)"><path d="${ridge({ amp: 10, floor: 460, seed: 10, waves: [300, 120], x0: -20, x1: W, y: 322 })}" fill="${p.ridge.far}"/>${pineRow(-10, 960, (x) => 322 + Math.sin(x / 90) * 6, 30, p.ridge.pines, 12)}${pineRow(1330, W + 10, (x) => 324 + Math.sin(x / 90) * 6, 30, p.ridge.pines, 13)}</g>`,
      depth: 5,
      name: 'ridge',
    },
    {
      body: [
        `<path filter="url(#lift)" d="${ridge({ amp: 9, floor: H, seed: 14, waves: [420, 190], x0: -20, x1: W, y: 360 })}" fill="${p.hill.back}"/>`,
        `<path filter="url(#lift)" d="${ribbon(homeCreek, creekWidth)}" fill="${p.creek.water}"/>`,
        pine(1250, 410, 150, p, 22),
      ].join(''),
      depth: 4,
      name: 'backhill',
    },
    {
      body: [
        `<path filter="url(#lift)" d="${ridge({ amp: 10, floor: H, seed: 15, waves: [520, 230], x0: -20, x1: W, y: 440 })}" fill="${p.hill.front}"/>`,
    ...
    },
    {
      body: [
        pine(-10, 560, 470, p, 31),
        pine(90, 540, 300, p, 32),
        pine(1610, 560, 420, p, 34),
      ].join(''),
      depth: 0.8,
      name: 'edge',
    },
...
export const homestead = (p: Palette) =>
  `${defs(p)}<clipPath id="frame"><rect width="${W}" height="${H}" rx="22"/></clipPath><g clip-path="url(#frame)">${homesteadLayers(
    p,
  )
    .map((l) => l.body)
...
const lanternPost = () =>
  `<circle cx="690" cy="430" r="80" fill="url(#warmth)" opacity=".7"/>${glowDot(690, 430, 2.4)}`;
