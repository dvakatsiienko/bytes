...
const roofY = (x: number) =>
  x < 800
    ? EAVE - ((x - L) / (800 - L)) * (EAVE - PEAK)
    : EAVE - ((R - x) / (R - 800)) * (EAVE - PEAK);
...
const shell = (p: Palette) => {
  ...
  const roof = `<path d="M${L - 40} ${EAVE + 12}L800 ${PEAK - 14}L${R + 40} ${EAVE + 12}L${R + 40} ${EAVE + 34}L800 ${PEAK + 10}L${L - 40} ${EAVE + 34}Z" fill="${p.cabin.roof}"/><path d="M800 ${PEAK - 14}L${R + 40} ${EAVE + 12}L${R + 40} ${EAVE + 34}L800 ${PEAK + 10}Z" fill="${p.cabin.roofLit}"/>${Array.from(
    { length: 5 },
    (_, i) => {
      const t = (i + 1) / 6;
  ...
  return {
    back,
    frame: `<g filter="url(#liftHi)">${logs(L)}${logs(R - 24)}${floors}</g>`,
    roof: `${smoke}<g filter="url(#liftHi)">${chimney}${roof}</g>`,
  };
};
...
const machineRoom = (p: Palette) => {
  const wheel = (x: number, y: number, r: number) =>
    `<circle cx="${x}" cy="${y}" r="${r}" fill="none" stroke="${p.iron}" stroke-width="4"/><circle cx="${x}" cy="${y}" r="${n(r * 0.3)}" fill="${p.brass.base}"/><path d="M${x - r} ${y}h${r * 2}M${x} ${y - r}v${r * 2}" stroke="${p.iron}" stroke-width="2"/>`;
...
const chordsHall = (p: Palette) => {
  ...
  const notes = `<g fill="${p.flora.lapis}" opacity="${p.isNight ? 0.9 : 0.75}">${[
    ...
    .map(
      ([x, y]) =>
        `<circle cx="${x}" cy="${y}" r="5"/><path d="M${(x ?? 0) + 4} ${y}v-18l8 4" stroke="${p.flora.lapis}" stroke-width="2" fill="none"/>`,
    )
    .join('')}</g>`;
  return `${hallWindow}${rug}${organ}${notes}${dino({ hasWren: !p.isNight, p, scale: 0.9, stance: 'stand', x: 668, y: FLOOR - 96 * 0.9 })}${tag(p, 600, 200, 'chords')}`;
};
...
export const workshop = (p: Palette) => {
  ...
  const stars = p.isNight
    ? Array.from(
        { length: 50 },
        () =>
          `<circle cx="${n(rand() * W)}" cy="${n(rand() * 300)}" r="${n(0.6 + rand())}" fill="#FFF3C4" opacity="${n(0.4 + rand() * 0.5)}"/>`,
      ).join('')
    : '';
  ...
  const hallGlow = p.isNight
    ? `<circle cx="760" cy="420" r="260" fill="url(#warmth)" opacity=".6"/>`
    : '';
  const body = [
    sky,
    stars,
