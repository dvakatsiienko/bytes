...
export const fireflies = (count: number, z: number) => {
  ...
  const material = new THREE.PointsMaterial({
    ...
    depthWrite: false,
    map: glowSprite('rgba(255,250,215,1)', 'rgba(255,217,120,.55)'),
    size: 0.8,
    transparent: true,
    vertexColors: true,
  });
  ...
  const update = (t: number) => {
    flies.forEach(({ base, phase, reach }, i) => {
      ...
      positions[i * 3 + 1] =
        base.y + Math.sin(tau * (2 * t + phase[1])) * reach * 0.5;
      positions[i * 3 + 2] = z;
...
export const fireGlow = (x: number, y: number, z: number) => {
  ...
  const light = new THREE.PointLight(0xff_a0_4a, 0, 3.2, 1.6);
  light.position.set(at.x, at.y, z + 0.3);
  const glow = new THREE.Sprite(
    new THREE.SpriteMaterial({
      blending: THREE.AdditiveBlending,
      depthWrite: false,
      map: glowSprite('rgba(255,210,120,.9)', 'rgba(242,140,60,.35)'),
      transparent: true,
    }),
  );
  ...
  const update = (t: number, strength: number) => {
    const flicker =
      0.82 +
      0.1 * Math.sin(tau * 3 * t) +
      0.06 * Math.sin(tau * 7 * t + 1.3) +
      0.04 * Math.sin(tau * 11 * t + 0.4);
...
export const smoke = (x: number, y: number, z: number, isNight: boolean) => {
  ...
  const map = glowSprite(
    isNight ? 'rgba(154,163,200,.8)' : 'rgba(255,255,255,.95)',
    isNight ? 'rgba(154,163,200,.45)' : 'rgba(255,255,255,.6)',
  );
  const puffs = Array.from(
    { length: 5 },
    () =>
      new THREE.Sprite(
        new THREE.SpriteMaterial({ depthWrite: false, map, transparent: true }),
      ),
  );
  const group = new THREE.Group();
  group.add(...puffs);
  const update = (t: number) => {
    puffs.forEach((puff, i) => {
      ...
      puff.position.set(
        at.x + Math.sin(tau * age) * 0.08 + age * 0.35,
        at.y + age * 1.1,
        z,
      );
...
export const windowLight = (x: number, y: number, z: number) => {
  const at = toWorld(x, y);
  const light = new THREE.PointLight(0xff_c7_66, 0, 2.6, 1.8);
  light.position.set(at.x, at.y, z + 0.25);
  return {
    object: light,
    update: (strength: number) => {
      light.intensity = 1.6 * strength;
    },
  };
};
...
export const embers = (x: number, y: number, z: number) => {
  ...
  const update = (t: number) => {
    for (const { sprite, phase, sway } of sparks) {
      ...
      sprite.position.set(
        at.x + sway * 0.25 * age + Math.sin(tau * (age * 2 + phase)) * 0.04,
        at.y + 0.1 + age * 0.9,
        z,
      );
