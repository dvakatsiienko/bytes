...
export const LatestInvoices = async () => {
    ...
    const latestInvoicesListJSX = latestInvoicesList.map((invoice, i) => {
        return (
            <div
                key = { invoice.id }
                className = { clsx('flex flex-row items-center justify-between border-gray-200 py-4', { 'border-t': i !== 0 }) }>
                <div className = 'flex items-center'>
                    ...
                    <div className = 'min-w-0'>
                        ...
                        <p className = 'hidden text-gray-500 text-sm sm:block'>{invoice.customer.email}</p>
                    </div>
                </div>
                <p className = { `${ lusitana.className } truncate font-medium text-sm md:text-base` }>
                    {invoice.amount}
                </p>
            </div>
        );
