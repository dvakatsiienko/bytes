...
function SidebarProvider({
  ...
  const [openMobile, setOpenMobile] = React.useState(__DEV__);
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((valueInner: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: investigate Cookie Store API and rewrite if needed
  ...
  const toggleSidebar = React.useCallback(() => {
    return isMobile
      ? setOpenMobile((openInner) => !openInner)
      : setOpen((openInner) => !openInner);
  }, [isMobile, setOpen]);
...
function Sidebar({
  ...
  if (collapsible === 'none') {
    return (
      <div
        className={cn(
          'flex h-full w-(--sidebar-width) flex-col bg-sidear text-sidebar-foreground',
          className,
        )}
        data-slot='sidebar'
        {...props}>
